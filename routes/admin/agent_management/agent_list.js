...
const { directus_list_orders, directus_count_orders, directus_retrieve_user, directus_list_agents } = require('../../../config/global_functions');
const router = express.Router();

const urlapi = getDirectusUrl();
const moment = getMoment();

const SERVICE_TYPE = "admin_agent_list"

const profile = DEFAULT_PROFILE_ADMIN;
const tabside = getTabSideBase(profile)
const idbloc = 4
const blocname = tabside[idbloc].texte
const pagename = "Liste des agents"
...
router.get('/', async function (req, res, next) {
  ...
  let agents = []
  let r_dts_agents = await directus_list_agents(filters)
  if (r_dts_agents.success) {
    agents = r_dts_agents.data
  }
